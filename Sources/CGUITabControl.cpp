...
//! constructor
CGUITab::CGUITab(s32 number, boost::shared_ptr<IGUIEnvironment> environment,
		...
		DrawBackground(false)
{
	...
	const boost::shared_ptr<const IGUISkin> skin = environment->getSkin();
	if (skin)
...
void CGUITab::refreshSkinColors()
{
	...
	{
		TextColor = getSharedEnvironment()->getSkin()->getColor(EGDC_BUTTON_TEXT);
	}
}
...
void CGUITab::draw()
{
	...
	if (skin && DrawBackground)
		skin->draw2DRectangle(getSharedThis(), BackColor, AbsoluteRect, &AbsoluteClippingRect);
...
void CGUITab::deserializeAttributes(io::IAttributes* in, io::SAttributeReadWriteOptions* options=0)
{
	...
	if (lockedParent && lockedParent->getType() == EGUIET_TAB_CONTROL)
	{
		boost::shared_ptr<CGUITab> lockedThis = getSharedThis<CGUITab>();

		boost::static_pointer_cast<CGUITabControl>(lockedParent)->addTab(lockedThis);
		if (isVisible())
			boost::static_pointer_cast<CGUITabControl>(lockedParent)->setActiveTab(lockedThis);
	}
}
...
//! constructor
CGUITabControl::CGUITabControl(boost::shared_ptr<IGUIEnvironment> environment,
	boost::shared_ptr<IGUIElement> parent, const core::rect<s32>& rectangle,
...
//! Adds a tab
boost::shared_ptr<IGUITab> CGUITabControl::addTab(const wchar_t* caption, s32 id)
{
	boost::shared_ptr<CGUITab> tab = boost::make_shared<CGUITab>(Tabs.size(), getSharedEnvironment(), getSharedThis(), calcTabPos(), id);
...
void CGUITabControl::addTab(boost::shared_ptr<CGUITab> tab)
{
	if (!tab)
		return;
...
boost::shared_ptr<IGUITab> CGUITabControl::insertTab(s32 idx, const wchar_t* caption, s32 id)
{
	if ( idx < 0 || idx > (s32)Tabs.size() )	// idx == Tabs.size() is indeed ok here as core::array can handle that
		return NULL;

	boost::shared_ptr<CGUITab> tab = boost::make_shared<CGUITab>(idx, getSharedEnvironment(), getSharedThis(), calcTabPos(), id);
...
boost::shared_ptr<IGUITab> CGUITabControl::getTab(s32 idx) const
{
...
s32 CGUITabControl::calcTabWidth(s32 pos, boost::shared_ptr<IGUIFont> font, const wchar_t* text, bool withScrollControl) const
{
	if ( !font )
		return 0;
...
void CGUITabControl::draw()
{
	...
	boost::shared_ptr<CGUITab> activeTab = 0;
	...
	for (u32 i=CurrentScrollTabIndex; i<Tabs.size(); ++i)
	{
		...
		{
			skin->draw3DTabButton(lockedThis, false, frameRect, &AbsoluteClippingRect, VerticalAlignment);
	...
	if (left != 0 && right != 0 && activeTab != 0)
	{
		...
		if ( VerticalAlignment == EGUIA_UPPERLEFT )
		{
			...
			skin->draw3DTabButton(lockedThis, true, frameRect, &AbsoluteClippingRect, VerticalAlignment);
	...
	skin->draw3DTabBody(lockedThis, Border, FillBackground, AbsoluteRect, &AbsoluteClippingRect, TabHeight, VerticalAlignment);
...
s32 CGUITabControl::getTabAt(s32 xpos, s32 ypos)
{
...
bool CGUITabControl::setActiveTab(boost::shared_ptr<IGUITab> tab)
{
	for (s32 i=0; i<(s32)Tabs.size(); ++i)
		if (Tabs[i] == tab)
...
//! Removes a child.
void CGUITabControl::removeChild(boost::shared_ptr<IGUIElement> child)
{
	bool isTab = false;
...
void CGUITabControl::setWeakThis(boost::shared_ptr<IGUIElement> value)
{
	...
	UpButton = lockedEnvironment->addButton(core::rect<s32>(0, 0, 10, 10), lockedThis);
	...
	if (UpButton)
	{
		...
		UpButton->setAlignment(EGUIA_LOWERRIGHT, EGUIA_LOWERRIGHT, EGUIA_UPPERLEFT, EGUIA_UPPERLEFT);
		UpButton->setOverrideFont(lockedEnvironment->getBuiltInFont());
	}

	DownButton = lockedEnvironment->addButton(core::rect<s32>(0, 0, 10, 10), lockedThis);
	...
	if (DownButton)
	{
		...
		DownButton->setAlignment(EGUIA_LOWERRIGHT, EGUIA_LOWERRIGHT, EGUIA_UPPERLEFT, EGUIA_UPPERLEFT);
		DownButton->setOverrideFont(lockedEnvironment->getBuiltInFont());
	}
