...
public:
	boost::shared_ptr<IMesh> createCubeMesh(const core::vector3df& size) const;

	boost::shared_ptr<IMesh> createHillPlaneMesh(
	...
	boost::shared_ptr<IMesh> createArrowMesh(const u32 tesselationCylinder,
	...
	boost::shared_ptr<IMesh> createSphereMesh(f32 radius, u32 polyCountX, u32 polyCountY) const;

	boost::shared_ptr<IMesh> createCylinderMesh(f32 radius, f32 length, u32 tesselation,
				const video::SColor& color=0xffffffff,
				bool closeTop=true, f32 oblique=0.f) const;

	boost::shared_ptr<IMesh> createConeMesh(f32 radius, f32 length, u32 tesselation,
				const video::SColor& colorTop=0xffffffff,
				const video::SColor& colorBottom=0xffffffff,
				f32 oblique=0.f) const;

	boost::shared_ptr<IMesh> createVolumeLightMesh(
