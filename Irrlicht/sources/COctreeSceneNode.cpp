...
void COctreeSceneNode::OnRegisterSceneNode()
{
	if (IsVisible)
	{
		...
		{
			const boost::shared_ptr<const video::IMaterialRenderer> rnd =
	...
	case video::EVT_STANDARD:
		{
			...
			for (u32 i=0; i<Materials.size(); ++i)
			{
				...
				const boost::shared_ptr<const video::IMaterialRenderer> rnd = driver->getMaterialRenderer(Materials[i].MaterialType);
